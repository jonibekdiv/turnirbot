...
module.exports = (bot) => {
  ...
  bot.action(/^host:open:(.+)$/, async (ctx) => {
    ...
    const kb = Markup.inlineKeyboard([
      ...
      [Markup.button.callback(t('host_room_resend'), CALLBACK.HOST_ROOM_RESEND + tour.id)],
    ...
    await safeEdit(
      ...
      `🎯 <b>${escapeHtml(tour.title)}</b>\n\n` +
        ...
        `📤 ${t('host_room_resend')}: <b>${roomStatus}</b>`,
    ...
    if (s === STATES.HOST_SEND_ROOM_ID) {
      ...
      return ctx.reply(
        `✅ <b>${t('host_room_id')}</b>\n\n` +
          ...
          `<i>${t('host_room_pass')} — ${t('success')}</i>`,
        { parse_mode: 'HTML', reply_markup: backToHostOpen(t, tid).reply_markup }
      );
    }
